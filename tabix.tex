...
\cline{2-7}
  & \multicolumn{6}{c|}{\textcolor{gray}{\it Optional dummy entry for stats (included in n\_bin)}} \\
\cline{3-7}
  & & \multicolumn{2}{l|}{\sf bin} & Dummy bin number & {\tt uint32\_t} & 37450 \\
\cline{3-7}
  & & \multicolumn{2}{l|}{\sf n\_chunk} & \# chunks & {\tt int32\_t} & 2 \\
\cline{3-7}
  & & \multicolumn{5}{c|}{\textcolor{gray}{\it First chunk for dummy bin}} \\
\cline{4-7}
  & & & {\sf unmapped\_beg} & (Virtual) file offset of the start of unmapped but placed & {\tt uint64\_t} & \\
\cline{4-7}
  & & & {\sf unmapped\_end} & (Virtual) file offset of the end of unmapped but placed & {\tt uint64\_t} & \\
\cline{4-7}
  & & \multicolumn{5}{c|}{\textcolor{gray}{\it Second chunk for dummy bin}} \\
\cline{4-7}
  & & & {\sf mapped\_count} & Number of mapped read-segments for this reference & {\tt uint64\_t} & \\
\cline{4-7}
  & & & {\sf unmapped\_count} & Number of unmapped read-segments for this reference & {\tt uint64\_t} & \\
\cline{2-7}
...
\multicolumn{4}{|l|}{\sf n\_no\_coor} & (Optional) \# unmapped reads without coordinates set & {\tt uint64\_t} & \\
\hline
\end{tabular}
\end{center}
